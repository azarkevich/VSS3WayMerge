...
namespace vcslib
{
	public class FileCache
	{
		...
		public void AddFileContent(string key, string content)
		{
			lock (this)
			{
				string oldDataPath = null;
				...
				var filePath = Path.Combine(store, string.Format("{0}-content", _indexEntriesCount));

				ce = new CacheEntry(key, filePath, null, null);
